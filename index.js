...
const getResults = (value) => {
    ...
    keywordsCategories.forEach( categorie => {
        let keywordsCleaned = categorie.keywords.map( element => cleanedKeyword(element))
        let index = -1
        keywordsCleaned.forEach( (kw,i) => {
            if (kw.includes(cleanedKeyword(value))) {
                index = i
            }
        })
        if (index >=0) {
            result.push(categorie.keywords[index])
            let inter = categorie.keywords.filter((value, i, array) => {
                return i != index
            })
            result = [...result,...inter]
        }
...
const reloadArticles = () => {
    ...
    const articlesToShow = data.articles.filter(element => {
        return currentKeywords.filter(tag => element.tags.includes(tag)).length>0
    });
...
const showKeywordsList = (value) => {
    ...
    if (value.length >= 3) {
        ...
        getResults(value).forEach( keyword => {
